...
// 데이터 전체 (일부 중략 없이 정교하게 추출된 리스트 사용)
const athletes = [
...
async function finalImport() {
    ...
    console.log(`Starting final import for ${athletes.length} athletes...`);

    for (const item of athletes) {
        const email = `${item.name}${Math.floor(Math.random() * 100)}@gla.com`;

        // 1. Auth User 생성 (Login용)
        ...
        if (authError) {
            console.error(`Error with ${item.name}:`, authError.message);
            continue;
        }

        // 2. Public.users 정보 업데이트
        ...
        if (dbError) {
            console.error(`DB Error for ${item.name}:`, dbError.message);
        } else {
