...
function updateLayout(filePath) {
    ...
    const layoutRegex = /\{\/\* Info row: Category \+ Venue \+ Date \+ Remarks \*\/\}[\s\S]*?<div className="flex items-center gap-2 bg-transparent dark:bg-zinc-800\/50 px-3 py-2\.5 rounded-xl border border-zinc-100 dark:border-zinc-800">[\s\S]*?<Users size={14} className="shrink-0 text-zinc-400" \/>[\s\S]*?<span className="text-xs text-zinc-600 dark:text-zinc-400 font-medium">[\s\S]*?\{t\.participants\.length > 0 \? t\.participants\.join\(\", \"\) : \"출장자 미지정\"\}[\s\S]*?<\/span>[\s\S]*?<\/div>/;

    const newLayout = `{/* Row 1: Category + Venue + Date + Vehicle */}
...
console.log("Refactored coach trip calendar layout");
