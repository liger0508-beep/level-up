...
import { Journal, JOURNAL_TYPE_LABELS } from "@/lib/journal-sync";

interface LinkedJournalCardProps {
...
export function LinkedJournalCard({ journal, onRemove, onMoreClick, readOnly = false }: LinkedJournalCardProps) {
    return (
            ...
            <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                    <span className="text-xs font-black px-2 py-0.5 rounded-md bg-brand-navy/10 text-brand-navy dark:bg-brand-navy/20 dark:text-brand-navy-light uppercase">
                        {journal ? (JOURNAL_TYPE_LABELS[journal.type] || "훈련 일지") : "훈련 일지"}
                    </span>
                </div>
                {!readOnly && onRemove && (
                    <button
