...
function VoiceReviewContent() {
  ...
  const navigateBack = () => {
      if (recordId) {
          router.replace(`/training/${recordId}`);
      } else {
          router.back();
      }
  };
      ...
      if (tType === 'review_hole' || tType === 'review_category') {
          ...
          if (isAllFinished) {
                   ...
                   try {
                       ...
                       if (chainStr) {
                           ...
                           if (currentIndexInChain !== -1 && currentIndexInChain + 1 < chain.length) {
                               ...
                               if (currentGroup === "second" && nextGroup === "tee") {
                                   shouldConnect = true;
                               }
                               ...
                               // If not shouldConnect, we break out and let it fall through to finalCompletionPhrase
                           }
                       }
                   } catch (e) {
               ...
               const getGroupName = (cat: string) => {
                   ...
                   if (["180m이상", "150-179m", "120-149m", "90-119m", "피치샷", "세컨샷", "아이언"].some(g => cat.includes(g))) return "세컨샷";
             ...
             if (stageIndex === parsedHoles.length - 1) {
                 ...
                 try {
                     ...
                     if (chainStr) {
                         ...
                         if (cIdx !== -1 && cIdx + 1 < chain.length) {
                             const nextCatName = chain[cIdx + 1].catName.toLowerCase();
                             const currentCatName = reviewCat?.toLowerCase() || "";
                             const getCatGroup = (cat: string) => {
                                 if (cat.includes("티샷")) return "tee";
                                 if (["180m이상", "150-179m", "120-149m", "90-119m", "피치샷", "아이언", "세컨샷"].some(g => cat.includes(g))) return "second";
                                 if (cat.includes("벙커") || cat.includes("어프로치") || cat.includes("그린주변") || cat.includes("숏게임")) return "short";
                                 if (cat.includes("퍼팅") || cat.includes("putt") || ["9m", "4-8m", "2-3m", "1m"].some(g => cat.includes(g))) return "putt";
                                 return "unknown";
                             };
                             const cg = getCatGroup(currentCatName);
                             const ng = getCatGroup(nextCatName);
                             if (cg === ng && cg !== "unknown") hasNextCat = true;
                             else if (cg === "second" && ng === "tee") hasNextCat = true;
                         }
                     }
                 } catch(e) {}
      ...
      if (tType === 'review_hole' || tType === 'review_category') {
          ...
          if (targetStr.includes("180") || targetStr.includes("150") || targetStr.includes("120") || targetStr.includes("90") || targetStr.includes("피치샷")) {
              ...
              if (!isPrep) {
                  ...
                  if (distNum >= 230) {
                      if (loc.includes("벙커") || loc === "GB" || loc === "FB") {
                          resultText = "벙커";
                      } else if (loc.includes("러프") || loc === "RO") {
                          resultText = "러프";
                      } else if (loc.includes("페널티") || loc.includes("패널티") || loc.includes("해저드") || loc.includes("벌타") || loc.includes("오비") || loc === "OB" || loc === "PA" || loc === "PS") {
                          resultText = "패널티";
                      } else {
                          resultText = remainDist ? `핀까지 ${convertDistToKorean(remainDist.trim())} 미터` : "페어웨이";
                      }
  ...
  return (
    ...
    >
       ...
       <div className="w-full max-w-sm flex flex-col items-center text-center relative">
           ...
           {/* ── 5. ACTION BUTTONS ── */}
           <div className="w-full flex justify-between gap-3 sm:gap-5 items-end mb-6 px-4 sm:px-8">
               {(!isStarted && trainingType === 'lesson_review') ? (
                   <button
                       onClick={(e) => { e.stopPropagation(); handleStartLessonReview(); }}
                       className="flex flex-col items-center group active:scale-95 transition-all flex-1"
                   >
                       <div className="w-full h-16 sm:h-20 bg-zinc-900 dark:bg-white rounded-2xl flex flex-col items-center justify-center gap-1 sm:gap-1.5 shadow-md group-hover:scale-95 transition-transform">
                           <Play className="w-5 h-5 sm:w-6 sm:h-6 text-white dark:text-zinc-900 fill-current ml-1" />
                           <span className="text-[12px] sm:text-[13px] font-bold text-white dark:text-zinc-900">시작</span>
                       </div>
                   </button>
               ) : (
                   <button
                       ...
                       className="flex flex-col items-center group active:scale-95 transition-all flex-1"
                   >
                       <div className={`w-full h-16 sm:h-20 rounded-2xl flex flex-col items-center justify-center gap-1 sm:gap-1.5 shadow-md group-hover:scale-95 transition-transform ${isPaused ? 'bg-zinc-900 dark:bg-white text-white dark:text-zinc-900' : 'bg-zinc-900 dark:bg-white text-white dark:text-zinc-900'}`}>
                           {isPaused ? <Play className="w-5 h-5 sm:w-6 sm:h-6 fill-current ml-1" /> : <Pause className="w-5 h-5 sm:w-6 sm:h-6 fill-current" />}
                           <span className="text-[12px] sm:text-[13px] font-bold text-white dark:text-zinc-900">{isPaused ? '재개' : '정지'}</span>
                       </div>
                   </button>
               )}
               ...
               <button
                   ...
                   className="flex flex-col items-center group active:scale-95 transition-all flex-1"
               >
                   <div className="w-full h-16 sm:h-20 bg-white dark:bg-zinc-800 border-2 border-zinc-100 dark:border-zinc-700 rounded-2xl flex flex-col items-center justify-center gap-1 sm:gap-1.5 shadow-sm group-hover:scale-95 transition-transform">
                       <Volume2 className="w-5 h-5 sm:w-6 sm:h-6 text-zinc-700 dark:text-zinc-300" />
                       <span className="text-[12px] sm:text-[13px] font-bold text-zinc-700 dark:text-zinc-300">듣기</span>
                   </div>
               </button>
               ...
               {trainingType !== 'lesson_review' ? (
                   <button 
                      ...
                      className="flex flex-col items-center group active:scale-95 transition-all flex-1"
                   >
                       <div className="w-full h-16 sm:h-20 bg-blue-600 rounded-2xl flex flex-col items-center justify-center gap-1 sm:gap-1.5 shadow-md group-hover:scale-95 transition-transform text-white">
                           <Check className="w-5 h-5 sm:w-6 sm:h-6" strokeWidth={3} />
                           <span className="text-[12px] sm:text-[13px] font-bold text-white">완료</span>
                       </div>
                   </button>
               ) : null}
               ...
               <button 
                   ...
                   className="flex flex-col items-center group active:scale-95 transition-all flex-1"
               >
                   <div className="w-full h-16 sm:h-20 bg-rose-50 dark:bg-rose-500/10 border-2 border-rose-100 dark:border-rose-500/20 rounded-2xl flex flex-col items-center justify-center gap-1 sm:gap-1.5 shadow-sm group-hover:scale-95 transition-transform text-rose-500">
                       <Square className="w-4 h-4 sm:w-5 sm:h-5 fill-current" />
                       <span className="text-[12px] sm:text-[13px] font-bold text-rose-500">종료</span>
                   </div>
               </button>
           </div>
