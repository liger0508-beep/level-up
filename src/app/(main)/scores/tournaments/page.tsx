    ...
    useEffect(() => {
        const load = async () => {
            ...
            if (tournamentsList.length > 0) {
                ...
                if (participants) {
                    const counts = participants.reduce((acc: Record<string, number>, p: any) => {
    ...
    return (
        <div className="p-4 sm:p-8 max-w-4xl mx-auto pb-24">
            <div className="flex items-center justify-between mb-8">
                ...
                {(userRole === 'admin' || userRole === 'superadmin' || userRole === 'coach' || userRole === 'headquarter' || userRole === 'office') && (
                                ...
                                >
                                    ...
                                    <div className="flex flex-col gap-1.5 ml-10">
                                        <div className="flex items-center justify-between">
                                            ...
                                            <div className="flex items-center gap-1.5 text-xs font-medium text-zinc-500 bg-zinc-100 dark:bg-zinc-800 px-2 py-0.5 rounded-md">
                                                <span className="text-zinc-400">참가</span>
                                                <span className="font-bold text-zinc-700 dark:text-zinc-300">{t.participant_count || 0}명</span>
                                            </div>
                                        </div>
                                        <div className="flex items-center gap-2 text-xs font-medium text-zinc-500">
                            ...
                            <table className="w-full text-sm">
                                <thead>
                                    <tr className="bg-zinc-50/50 dark:bg-zinc-800/50 border-b border-zinc-200 dark:border-zinc-700 text-left text-zinc-500 dark:text-zinc-400">
                                        ...
                                        <th className="py-2.5 px-4 font-semibold text-center w-auto">대회명</th>
                                        ...
                                        <th className="py-2.5 px-4 font-semibold text-center w-24">참가 인원</th>
                                ...
                                <tbody className="divide-y divide-zinc-100 dark:divide-zinc-800">
                                    {tournaments.map((t, idx) => (
                                        <tr
                                            key={t.id}
                                            onClick={() => router.push(`/scores/tournaments/${t.id}`)}
                                        ...
                                        >
                                            ...
                                            <td className="py-3.5 px-4 text-center text-zinc-600 dark:text-zinc-400">
                                                <span className="font-bold text-zinc-700 dark:text-zinc-300">{t.participant_count || 0}</span>명
                                            </td>
                                            <td className="py-3.5 px-4 text-center">
