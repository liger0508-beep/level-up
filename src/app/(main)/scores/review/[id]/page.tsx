...
const formatScore = (num: number | undefined) => {
    if (num === undefined || num === null) return "";
    return (Math.sign(num) * Math.round(Math.abs(num) * 1000) / 1000).toFixed(3);
};
            ...
            <div className="max-w-[1600px] mx-auto">
                ...
                <TableSection title="샷별 분석 지수" range="C51:V63" startRow={51}>
                    ...
                    <tr key="shot-sg-total" className="bg-zinc-50 dark:bg-zinc-900/50 font-bold">
                        ...
                        {holeNums.map(n => <td key={n} className="border border-zinc-200 dark:border-zinc-800 p-2 text-center">{formatScore(getAnalysis(n)?.totalSG)}</td>)}
                    </tr>
                </TableSection>
                ...
                <TableSection title="거리별 분석 지수" range="C69:U86" startRow={69}>
                    <tr key="dist-sg-total">
                        ...
                        {holeNums.map(n => <td key={n} className="border border-zinc-200 dark:border-zinc-800 p-2 text-center font-bold">{formatScore(getAnalysis(n)?.totalSG)}</td>)}
                    </tr>
                    <tr key="dist-sg-driver-dist">
                        ...
                        {holeNums.map(n => <td key={n} className="border border-zinc-200 dark:border-zinc-800 p-2 text-center text-sm">{formatScore(getAnalysis(n)?.summary?.distSG_DriverDist)}</td>)}
                    </tr>
                    <tr key="dist-sg-driver-acc">
                        ...
                        {holeNums.map(n => <td key={n} className="border border-zinc-200 dark:border-zinc-800 p-2 text-center text-sm">{formatScore(getAnalysis(n)?.summary?.distSG_DriverAcc)}</td>)}
                    </tr>
                    <tr key="dist-sg-180">
                        ...
                        {holeNums.map(n => <td key={n} className="border border-zinc-200 dark:border-zinc-800 p-2 text-center text-sm">{formatScore(getAnalysis(n)?.summary?.distSG_180Plus)}</td>)}
                    </tr>
                    <tr key="dist-sg-150-179">
                        ...
                        {holeNums.map(n => <td key={n} className="border border-zinc-200 dark:border-zinc-800 p-2 text-center text-sm">{formatScore(getAnalysis(n)?.summary?.distSG_150_179)}</td>)}
                    </tr>
                    <tr key="dist-sg-120-149">
                        ...
                        {holeNums.map(n => <td key={n} className="border border-zinc-200 dark:border-zinc-800 p-2 text-center text-sm">{formatScore(getAnalysis(n)?.summary?.distSG_120_149)}</td>)}
                    </tr>
                    <tr key="dist-sg-90-119">
                        ...
                        {holeNums.map(n => <td key={n} className="border border-zinc-200 dark:border-zinc-800 p-2 text-center text-sm">{formatScore(getAnalysis(n)?.summary?.distSG_90_119)}</td>)}
                    </tr>
                    <tr key="dist-sg-pitch">
                        ...
                        {holeNums.map(n => <td key={n} className="border border-zinc-200 dark:border-zinc-800 p-2 text-center text-sm">{formatScore(getAnalysis(n)?.summary?.distSG_Pitch31_89)}</td>)}
                    </tr>
                    <tr key="dist-sg-bunker">
                        ...
                        {holeNums.map(n => <td key={n} className="border border-zinc-200 dark:border-zinc-800 p-2 text-center text-sm">{formatScore(getAnalysis(n)?.summary?.distSG_Bunker)}</td>)}
                    </tr>
                    <tr key="dist-sg-approach">
                        ...
                        {holeNums.map(n => <td key={n} className="border border-zinc-200 dark:border-zinc-800 p-2 text-center text-sm">{formatScore(getAnalysis(n)?.summary?.distSG_Approach)}</td>)}
                    </tr>
                    <tr key="dist-sg-putt-9">
                        ...
                        {holeNums.map(n => <td key={n} className="border border-zinc-200 dark:border-zinc-800 p-2 text-center text-sm">{formatScore(getAnalysis(n)?.summary?.distSG_Putt9Plus)}</td>)}
                    </tr>
                    <tr key="dist-sg-putt-4-8">
                        ...
                        {holeNums.map(n => <td key={n} className="border border-zinc-200 dark:border-zinc-800 p-2 text-center text-sm">{formatScore(getAnalysis(n)?.summary?.distSG_Putt4_8)}</td>)}
                    </tr>
                    <tr key="dist-sg-putt-2-3">
                        ...
                        {holeNums.map(n => <td key={n} className="border border-zinc-200 dark:border-zinc-800 p-2 text-center text-sm">{formatScore(getAnalysis(n)?.summary?.distSG_Putt2_3)}</td>)}
                    </tr>
                    <tr key="dist-sg-putt-1">
                        ...
                        {holeNums.map(n => <td key={n} className="border border-zinc-200 dark:border-zinc-800 p-2 text-center text-sm">{formatScore(getAnalysis(n)?.summary?.distSG_Putt1)}</td>)}
                    </tr>
                ...
                <TableSection title="상세 분석 데이터" range="C92:U150" startRow={92}>
                    ...
                    <tr key="total-sg">
                        ...
                        {holeNums.map(n => <td key={n} className="border border-zinc-200 dark:border-zinc-800 p-2 text-center font-black bg-zinc-50 dark:bg-zinc-800/50">{formatScore(getAnalysis(n)?.totalSG)}</td>)}
                    </tr>
                        ...
                        rows.push(
                            <tr key={`analysis-shot-try-pos-${s}`}>
                                ...
                                {holeNums.map(n => {
                                    ...
                                    return <td key={n} className="border border-zinc-200 dark:border-zinc-800 p-2 text-center">{formatScore(val)}</td>
                                })}
                            </tr>,
                            ...
                            <div className="p-4">
                                <table className="w-full text-[12px] border-collapse border border-zinc-200 dark:border-zinc-800">
                                    ...
                                    <tbody className="divide-y divide-zinc-200 dark:divide-zinc-800">
                                        {/* General Stats */}
                                        {[
                                            { label: "플레이 내용", value: formatScore(summary.playContent) },
                                            { label: "내용대비 스코어", value: (summary.scoreVsContent > 0 ? "+" : "") + formatScore(summary.scoreVsContent), color: summary.scoreVsContent > 0 ? "text-blue-500" : "text-red-500" },
                                            { label: "롱게임대비 숏게임", value: (summary.longVsShort > 0 ? "+" : "") + formatScore(summary.longVsShort), color: summary.longVsShort > 0 ? "text-blue-500" : "text-red-500" },
                                        ...
                                        {summary.categories.map((cat, idx) => (
                                            <tr key={`cat-${idx}`} className="hover:bg-zinc-50 dark:hover:bg-zinc-800/30">
                                                ...
                                                <td className={cn("border border-zinc-200 dark:border-zinc-800 p-2 text-center font-bold", cat.sg > 0 ? "text-blue-500" : cat.sg < 0 ? "text-red-500" : "text-zinc-400")}>
                                                    {cat.sg !== 0 ? (cat.sg > 0 ? "+" : "") + formatScore(cat.sg) : "0.0"}
                                                </td>
