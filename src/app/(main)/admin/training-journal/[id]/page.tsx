...
export default function JournalDetailPage() {
    ...
    useEffect(() => {
        async function loadData() {
            ...
            setIsLoading(true);
            try {
                const [data, listData, commentData] = await Promise.all([
                    fetchJournalById(id as string),
                    fetchJournals(),
    ...
    const handleDelete = async () => {
        setIsMenuOpen(false);
        if (window.confirm("훈련일지를 삭제하시겠습니까?")) {
            try {
    ...
    if (!journal) return <div className="min-h-screen bg-zinc-50 dark:bg-zinc-950 flex items-center justify-center text-zinc-400">일지를 찾을 수 없습니다.</div>;

    const styles = JOURNAL_TYPE_COLORS[journal.type];

    // Filtered journals for search panel
    ...
    const TYPE_FILTERS: { key: JournalType; label: string }[] = [
        { key: "all", label: "전체" },
        { key: "good", label: "굿샷" },
        { key: "miss", label: "미스샷" },
        { key: "field", label: "필드 노트" },
    ];
    ...
    return (
        <div className="min-h-screen bg-zinc-50 dark:bg-zinc-950 pb-20">
                ...
                <div className="max-w-3xl mx-auto px-4 sm:px-8 h-16 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <h1 className="text-lg font-bold text-zinc-900 dark:text-zinc-50">훈련일지</h1>
                        </div>
                    </div>
                    ...
                    <div className="relative">
                        ...
                        {isMenuOpen && (
                            <div className="absolute right-0 mt-2 w-36 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl shadow-lg overflow-hidden z-50">
                                <Link
                                    href={`/admin/training-journal/${id}/edit`}
            ...
            <main className="max-w-3xl mx-auto px-4 sm:px-8 py-6 space-y-5">
                    ...
                    <div className="flex items-center gap-4 pt-4 border-t border-zinc-100 dark:border-zinc-800/50">
                        <div className="flex items-center gap-2 flex-1">
                            ...
                            <div>
                                ...
                                <p className="text-sm font-semibold text-zinc-900 dark:text-zinc-100">{journal.author}</p>
                            </div>
                        </div>
                ...
                {/* ── 5. Filter + Search ── */}
                <section className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl shadow-sm overflow-hidden">
                    <div className="flex items-center justify-between px-5 pt-4 pb-3 border-b border-zinc-100 dark:border-zinc-800 gap-3 flex-wrap">
                        {/* Type Filters */}
                    ...
                    {showSearch && (
                        <div className="px-5 py-3 border-b border-zinc-100 dark:border-zinc-800">
                            <input
                                ...
                                autoFocus
                                className="w-full px-4 py-2 rounded-xl border border-zinc-200 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-800 text-sm text-zinc-900 dark:text-zinc-100 placeholder:text-zinc-400 focus:outline-none focus:ring-2 focus:ring-brand-navy/40 transition-all"
                            />
                        </div>
                    )}
                    ...
                    <div className="divide-y divide-zinc-100 dark:divide-zinc-800 max-h-64 overflow-y-auto">
                        {filteredJournals.length > 0 ? (
                            filteredJournals.map((j) => {
                                const s = JOURNAL_TYPE_COLORS[j.type];
                                const isCurrent = j.id === journal.id;
                                return (
                                    <Link
                                        key={j.id}
                                        href={`/admin/training-journal/${j.id}`}
                                        className={cn(
                                            "flex items-center gap-3 px-5 py-3.5 transition-colors",
                                            isCurrent
                                                ? "bg-brand-navy/5 dark:bg-brand-navy/10"
                                                : "hover:bg-zinc-50 dark:hover:bg-zinc-800/50"
                                        )}
                                    >
                                        <span className={cn("shrink-0 inline-flex items-center justify-center w-14 px-2 py-0.5 rounded-md text-[10px] font-bold", s.bg, s.text)}>
                                            {JOURNAL_TYPE_LABELS[j.type]}
                                        </span>
                                        <span className={cn("flex-1 text-sm truncate", isCurrent ? "font-bold text-brand-navy dark:text-brand-navy-light" : "text-zinc-700 dark:text-zinc-300")}>
                                            {j.content.replace(/<[^>]*>/g, "").trim() || "내용 없음"}
                                        </span>
                                        <span className="text-[11px] text-zinc-400 shrink-0">{j.date.slice(5).replace("-", ".")}</span>
                ...
                <div className="flex justify-center pt-2">
                    <button
                        onClick={() => router.push("/admin/training-journal")}
