    ...
    function getTrainingCategories(isPrep: boolean, reviewData: any) {
        ...
        if (isPrep) {
            ...
            return positiveCats.map(cat => {
                ...
                let candidateShots: any[] = [];

                reviewData.analysis.forEach((h: any) => {
                    let holeSg = 0;
                    if (cat.name === "퍼팅") {
                        const fields = field as string[];
                        holeSg = fields.reduce((sum, f) => sum + (h.summary[f] || 0), 0);
                    } else if (cat.name === "티샷") {
                        holeSg = hasBadTeeShot(h) ? threshold : 0;
                        const tShots = getCategoryShots("티샷", h, false);
                        if (tShots.some((s: any) => s.shotSG >= 0.2)) holeSg = threshold;
                    } else {
                ...
                candidateShots.forEach(shot => {
                    let groupId = "default";
                    if (cat.name === "퍼팅") {
                        const dist = shot.attemptDistance;
                        if (dist < 2) groupId = "1m";
                        else if (dist <= 3) groupId = "2~3m";
                        else if (dist <= 8) groupId = "4~8m";
                        else groupId = "9m+";
                    } else if (cat.name === "숏게임") {
                        const isBunker = shot.shotLabel.split('/')[0].trim().toUpperCase() === "GB";
                        if (isBunker) groupId = "bunker";
                        else {
                            const dist = shot.attemptDistance;
                            if (dist <= 10) groupId = "<=10m";
                            else if (dist <= 20) groupId = "11~20m";
                            else if (dist <= 30) groupId = "21~30m";
                            else groupId = "other";
                        }
                    } else if (cat.name === "아이언&피치샷") {
                        const dist = shot.attemptDistance;
                        if (dist <= 60) groupId = "31~60m";
                        else if (dist <= 90) groupId = "61~90m";
                        else if (dist <= 120) groupId = "91~120m";
                        else if (dist <= 150) groupId = "121~150m";
                        else if (dist <= 180) groupId = "151~180m";
                        else groupId = "181m+";
                    } else if (cat.name === "티샷") {
                        const hn = shot.holeNumber;
                        if (hn <= 3) groupId = "1~3";
                        else if (hn <= 6) groupId = "4~6";
                        else if (hn <= 9) groupId = "7~9";
                        else if (hn <= 12) groupId = "10~12";
                        else if (hn <= 15) groupId = "13~15";
                        else groupId = "16~18";
                    }
                    shot.groupId = groupId;
                });

                candidateShots.sort((a, b) => {
                    if (b.shotSG !== a.shotSG) return b.shotSG - a.shotSG;
                    return a.holeNumber - b.holeNumber;
                });
                ...
                for (const shot of candidateShots) {
                    if (!seenGroups.has(shot.groupId) && selectedShots.length < 5) {
                        selectedShots.push(shot);
                        seenGroups.add(shot.groupId);
                    }
                }

                for (const shot of candidateShots) {
                    if (selectedShots.length >= 5) break;
                    if (!selectedShots.some(s => s.uniqueId === shot.uniqueId)) {
                        selectedShots.push(shot);
                    }
                }

                if (cat.name === "티샷" || cat.name === "퍼팅") {
                    selectedShots.sort((a, b) => a.holeNumber - b.holeNumber);
                } else if (cat.name === "숏게임") {
                    selectedShots.sort((a, b) => {
                        const isBunkerA = a.groupId === "bunker";
                        const isBunkerB = b.groupId === "bunker";
                        if (isBunkerA && !isBunkerB) return 1;
                        if (!isBunkerA && isBunkerB) return -1;
                        return a.holeNumber - b.holeNumber;
                    });
                } else if (cat.name === "아이언&피치샷") {
                    selectedShots.sort((a, b) => a.attemptDistance - b.attemptDistance);
                }

                return { name: cat.name, holes: selectedShots };
            }).filter(cat => cat.holes.length > 0);
        }
    }
        ...
        if (autoStart) {
            for (let i = 0; i < currentCats.length; i++) {
                ...
                if (uncompletedHoles.length > 0) {
                    ...
                    router.replace(`/admin/training-temp/voice-guide?type=review_category&cat=${encodeURIComponent(catName)}&holes=${encodedHoles}&recordId=${training.id}&isPrep=${isPrep}`);
                    return;
                }
            }
        }
    ...
    const handleDelete = async () => {
        ...
        if (window.confirm("훈련 내용을 삭제하시겠습니까?")) {
            ...
            if (error) {
                ...
                alert("삭제되었습니다.");
                router.push("/admin/training-temp");
            }
        }
    };
        ...
        <div className="min-h-screen bg-zinc-50 dark:bg-zinc-950 pb-24">
            ...
            <header className="sticky top-0 z-40 bg-zinc-50/80 dark:bg-zinc-950/80 backdrop-blur-md border-b border-zinc-200 dark:border-zinc-800">
                <div className="max-w-3xl mx-auto px-4 h-16 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        <button onClick={() => router.push('/admin/training-temp')} className="p-2 -ml-2 rounded-full hover:bg-zinc-200 dark:hover:bg-zinc-800 text-zinc-900 dark:text-zinc-50 transition-colors">
                            <ChevronLeft size={24} />
                        </button>
                        <div className="flex items-center gap-2">
                            <Layers size={18} className="text-brand-navy dark:text-brand-navy-light shrink-0" />
                            <PageTitle>훈련 상세</PageTitle>
                        </div>
                    </div>
            ...
            <main className="max-w-3xl mx-auto px-4 py-6 space-y-6">
                ...
                <section className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 p-6 rounded-3xl shadow-sm space-y-4">
                    ...
                    <SectionTitle>{derivedTitle}</SectionTitle>
                                                                                    ...
                                                                                    <div className="p-4 sm:p-5">
                                                                                        <div className="bg-zinc-50/80 dark:bg-zinc-800/30 rounded-xl p-4 space-y-4 border border-zinc-100 dark:border-zinc-800/50 overflow-hidden">
                                                                                            ...
                                                                                            {selectedHoleData.worstShotInfo?.score != null && (() => {
                                                                                                const score = selectedHoleData.worstShotInfo.score;
                                                                                                const isPrep = training.title?.includes("[예습]");
                                                                                                let scoreColorClass = "text-blue-500";
                                                                                                ...
                                                                                                if (isPrep) {
                                                                                                    if (score > 0) {
                                                                                                        ...
                                                                                                        scoreColorClass = "text-brand-red";
                                                                                                        dotColorClass = "bg-brand-red";
                                                                                                    } else {
                                                                                                        scoreColorClass = "text-zinc-500";
                                                                                                        dotColorClass = "bg-zinc-500";
                                                                                                    }
                                                                                                }
                                                                                                return (
                                                                                                    <div className="flex items-center pt-4 border-t border-zinc-200/50 dark:border-zinc-700/50">
                                                                                                        <div className={cn("w-1 h-3.5 rounded-full mr-3 shrink-0", dotColorClass)}></div>
                                                                                                        <span className="text-sm font-medium text-zinc-500 w-10 shrink-0">점수</span>
                                                                                                        <span className={cn("text-[13px] sm:text-sm font-medium truncate flex-1", scoreColorClass)}>
                                                                                            ...
                                                                                            <button
                                                                                                onClick={(e) => {
                                                                                                    ...
                                                                                                    const executeTraining = async (isRetrain: boolean) => {
                                                                                                        ...
                                                                                                        const subsequentCats = isRetrain ? [currentCats[catIndex]] : currentCats.slice(catIndex >= 0 ? catIndex : 0);
                                                                                                        ...
                                                                                                        const encodedHoles = encodeURIComponent(JSON.stringify(targetHoles));
                                                                                                        sessionStorage.setItem('lastTrainingCat', catName);
                                                                                                        router.push(`/admin/training-temp/voice-guide?type=review_category&cat=${encodeURIComponent(catName)}&holes=${encodedHoles}&recordId=${id}&isPrep=${isPrep}`);
                                                                                                    };
                    ...
                    <section className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 p-6 rounded-3xl shadow-sm space-y-3">
                        ...
                        {reviewData && (
                            <div className="pt-2">
                                ...
                                <div className="w-full h-12 bg-zinc-100 dark:bg-zinc-800 rounded-2xl flex items-center justify-center shadow-inner border border-zinc-200 dark:border-zinc-700">
                                    <span className="text-sm font-black text-brand-navy">{formatTotalTimeDisplay(totalTrainingSeconds)}</span>
                                </div>
                            </div>
                        )}
                            ...
                            <button
                                onClick={() => {
                                    if (training.type === 'lesson_review' && training.templates?.[0]) {
                                        ...
                                        if (method === 'camera') {
                                            router.push(`/admin/training-temp/swing-test?recordId=${training.id}&type=lesson_review`);
                                        } else {
                                            router.push(`/admin/training-temp/voice-guide?recordId=${training.id}&type=lesson_review`);
                                        }
                                    } else if (training.type === 'swing_pose') {
                                        router.push(`/admin/training-temp/swing-pose?recordId=${training.id}`);
                                    } else {
                            ...
                            <div className="flex items-center justify-between">
                                <div className="space-y-0.5">
                                    ...
                                    <div className="flex items-center gap-1.5 mt-1.5 px-2 py-0.5 w-fit rounded bg-zinc-100 dark:bg-zinc-800 text-[10px] font-bold text-zinc-500">
                                        <Calendar size={10} />
                                        {reviewData.recentScore.date.slice(5).replace(/-/g, ".")}
                                    </div>
                                </div>
                ...
                <section className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl shadow-sm overflow-hidden mb-8">
                    ...
                    <div className="border-t border-zinc-100 dark:border-zinc-800 px-4 py-3 space-y-2">
                        ...
                        />
                        <div className="flex items-center justify-between">
                            <FileUploadButton iconOnly icon={<Paperclip size={18} />} onChange={handleCommentFileChange} />
                            <button
