...
import { ScorecardSelectionModal } from "@/components/training/ScorecardSelectionModal";
import { SwingKeyPreviewModal } from "@/components/training/SwingKeyPreviewModal";
...
function CreateTrainingContent() {
    ...
    const [showSwingKeyPreview, setShowSwingKeyPreview] = useState(false);
    ...
    // Scorecard Selection Modal
    const [showScoreModal, setShowScoreModal] = useState(false);
    const [scoreModalType, setScoreModalType] = useState<"prep" | "review">("prep");
    ...
    useEffect(() => {
        const loadRecent = async () => {
            ...
            try {
                ...
                const journals = await fetchJournalsByAthlete(lastPlayer);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (selectedTrainingType === "basic" && selectedTemplates.length === 0) {
    ...
    const handleScorecardSelect = async (scorecardId: string, playerName: string) => {
        if (!termStart || !termEnd || !selectedPart) {
            alert("파트와 훈련 기간을 먼저 입력해주세요.");
        ...
        setShowScoreModal(false);
        try {
            const typeLabel = scoreModalType === "prep" ? "예습" : "복습";
            const finalTitle = `[${typeLabel}] 스코어 분석 훈련`;

            const { error } = await saveTrainingRecord({
                playerName: playerName,
                category: scoreModalType === "prep" ? "preview" : "review",
                title: finalTitle,
                content: trainingComment,
                date: trainingDate,
                startTime: trainingTime,
                ...
                type: "training_temp",
                template_settings: [{
                    type: scoreModalType === "prep" ? "prep_scorecard" : "review_scorecard",
                    scorecardId: scorecardId
                }]
            });
            if (error) throw new Error(error);

            alert(`${playerName} 선수의 ${typeLabel} 훈련이 등록되었습니다.`);
            sessionStorage.removeItem(DRAFT_KEY);
            router.push("/admin/training-temp");
            router.refresh();
        } catch (error: any) {
        ...
        <div className="min-h-screen bg-zinc-50 dark:bg-zinc-950 px-4 sm:px-8 py-6">
            <div className="max-w-3xl mx-auto">
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {trainingTypeOptions.map((opt) => (
                                        <button
                                            ...
                                            type="button"
                                            onClick={() => {
                                                if (opt.key === "prep" || opt.key === "review") {
                                                    setScoreModalType(opt.key as "prep" | "review");
                                                    setShowScoreModal(true);
                                                    setSelectedTrainingType(opt.key);
                                                } else {
                                                    setSelectedTrainingType(opt.key);
                                                }
                                            }}
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        {lessonComments.length < 5 ? (
                                            <div className="flex justify-between items-center mt-2">
                                                <button
                                                    type="button"
                                                    onClick={() => setShowSwingKeyPreview(true)}
                                                    className="flex items-center gap-1.5 text-xs font-bold text-zinc-600 dark:text-zinc-400 hover:text-brand-navy transition-colors px-3 py-2 rounded-lg bg-zinc-100 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700"
                                                >
                                                    <Play size={14} fill="currentColor" /> 시뮬레이션 미리듣기
                                                </button>
                                                <button
                                                    type="button"
                ...
                <ScorecardSelectionModal
                    ...
                    title={scoreModalType === "prep" ? "예습 스코어 선택" : "복습 스코어 선택"}
                />

                <SwingKeyPreviewModal
