...
export default function AthleteReportPage() {
    ...
    useEffect(() => {
        ...
        async function fetchAthleteMonthlyData() {
            ...
            try {
                ...
                // Fetch coach feedback from player_reports
                const { data: reportData } = await supabase
                    .from("player_reports")
                    .select("content, coach_id")
                ...
                if (reportData?.coach_id) {
                    const { data: cData } = await supabase.from("users").select("name").eq("id", reportData.coach_id).maybeSingle();
                    setReportCoachName(cData?.name || "");
                } else {
                    setReportCoachName("");
                }
    ...
    const handleSaveReport = async () => {
        ...
        try {
            ...
            const { data, error } = await supabase.from('player_reports').upsert({
        ...
        <div className="min-h-screen bg-zinc-50 dark:bg-zinc-950 pb-20 print:bg-white print:pb-0">
            ...
            <main className="max-w-5xl mx-auto px-4 sm:px-8 py-8 space-y-8 print:py-0 print:px-0 [word-break:keep-all]">
                ...
                {selectedAthleteId ? (
                    ...
                    <div className="flex flex-col items-center justify-center py-32 text-center bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-[2.5rem] shadow-sm">
                        ...
                        <SectionTitle>선수를 선택해주세요</SectionTitle>
                        <p className="text-sm font-bold text-zinc-500 max-w-sm">
                            상단의 검색창에서 리포트를 열람할 선수를 선택하면,<br />
                            해당 월의 리포트를 확인할 수 있습니다.
                        </p>
                    </div>
                )}
            </main>
