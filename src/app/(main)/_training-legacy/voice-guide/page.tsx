...
const convertDistToKorean = (str: string) => {
  ...
  const generatePrompt = (stageIndex: number) => {
      ...
      if (tType === 'review_hole' || tType === 'review_category') {
          ...
          if (actualPar === 4 || (!actualPar && ["1", "2", "5", "7", "11", "14", "16", "18"].includes(hStr))) {
              parString = "4";
              parReading = "포";
          } else if (actualPar === 3 || (!actualPar && ["3", "8", "12", "17"].includes(hStr))) {
              parString = "3";
              parReading = "쓰리";
          } else {
              parString = "5";
          ...
          if (targetStr.includes("티샷")) {
              if (isPrep) {
                  ...
                  if (parString === "5" && remainDist <= 230) {
                      spoken = `${holeText}번홀, 투온이 가능한 파 파이브입니다. 정확하게 멀리 쳐주세요.`;
                      visual = `${holeText}번홀 투온 찬스 - 티샷 집중`;
                  } else if (parString === "4" && remainDist <= 89) {
                      spoken = `${holeText}번홀, 버디 확률이 높은 파 포 입니다. 티샷에 집중해 주세요.`;
                  ...
                  if (wind) {
                      ...
                      spoken = `${holeText}번홀, 파 ${parReading}, 티샷, 바람은 없습니다.`;
                      visual = `${holeText}번홀 - Par ${parString} - 바람 없음`;
                  }
                  ...
                  if (targetStr === "티샷 비거리") {
                      ...
                      let locGoal = "페어웨이 적중을 목표로 훈련해 주세요.";
                      if (loc.includes("벙커")) locGoal = "벙커를 피하는 공략을 목표로 훈련해 주세요.";
                      else if (loc.includes("페어웨이")) locGoal = "거리 확보를 목표로 티샷해주세요.";
                      else if (loc.includes("숲")) locGoal = "다음 샷을 하기 좋은 위치를 목표로 훈련해 주세요.";
                      else if (loc.includes("페널티") || loc.includes("패널티") || loc.includes("해저드") || loc.includes("벌타")) locGoal = "안전한 티샷 공략을 목표로 훈련해 주세요.";
                      else if (loc.includes("오비") || loc === "OB" || loc === "PA") locGoal = "인플레이를 목표로 훈련해 주세요.";
                      
                      if (loc.includes("페어웨이") && !loc.includes("벙커")) {
                          let distText = remainDist ? `${convertDistToKorean(remainDist.trim())} 미터` : "알 수 없는 거리";
                          spoken = `${holeText}번홀 티샷 결과는 ${distText} 였습니다, ${locGoal}`;
                      } else {
                          spoken = `${holeText}번홀 티샷 결과는 ${loc} 였습니다. ${locGoal}`;
                      }
                      visual = locGoal;
                  }
                  return { spoken: prefixSpoken + spoken, visual };
          ...
          if (targetStr.includes("180") || targetStr.includes("150") || targetStr.includes("120") || targetStr.includes("90") || targetStr.includes("피치샷")) {
              ...
              if (!isPrep) {
                  ...
                  spoken = `${holeText}번홀 ${distToRead} 미터 샷 결과는 ${resultSentence}`;
          ...
          if (targetStr.includes("어프로치") || targetStr.includes("그린주변샷") || targetStr.includes("숏게임")) {
              ...
              if (!isPrep) {
                  ...
                  let resultSentence = "";
                  if (!loc.includes("그린")) {
                      resultSentence = `온그린 미스 였습니다. ${subGoal}`;
                  ...
                  spoken = `${holeText}번홀, ${attemptLoc}, ${distStr} 미터 샷 결과는 ${resultSentence}`;
              } else {
          ...
          if (targetStr.includes("퍼팅") || targetStr.includes("putt") || ["9M이상", "4-8M", "2-3M", "1M"].includes(targetStr.toUpperCase())) {
              ...
              if (!isPrep) {
                  const ch = parsedHoles[stageIndex];
                  const resText = ch?.result || "";
                  const remainDist = resText.replace('m', '').trim() || "알수없음";
                  
                  visual = `${holeText}번홀 - ${distStr}m`;
                  
                  const numericDist = parseInt(distStr, 10) || 0;
                  if (numericDist >= 9) {
                      spoken = `${holeText}번홀 ${distStr} 미터 결과는 ${remainDist} 미터 였습니다. 1 미터 이내를 목표로 훈련을 진행해 주세요.`;
                  } else {
                      ...
                      const isBirdieOrEagle = (puttType === "이글" || puttType === "버디");
                      
                      if (numericDist >= 4 && numericDist <= 8) {
                          if (isBirdieOrEagle) {
                              spoken = `${holeText}번홀 ${distStr} 미터 지점에서 원퍼트를 목표로 훈련을 진행해 주세요.`;
                          } else {
                              spoken = `${holeText}번홀 ${distStr} 미터 지점과 같은 경사에서 원퍼트를 목표로 훈련을 진행해 주세요.`;
                          }
                      } else if (numericDist >= 2 && numericDist <= 3) {
                          if (isBirdieOrEagle) {
                              spoken = `${holeText}번홀 ${distStr} 미터 지점에서 찬스를 놓쳤습니다. 반드시 성공하는 것을 목표로 훈련을 진행해 주세요.`;
                          } else {
                              spoken = `${holeText}번홀 ${distStr} 미터 지점에서 세이브를 하지 못했습니다. 반드시 성공하는 것을 목표로 훈련을 진행해 주세요.`;
                          }
                      } else {
                          // 1m 이하
                          if (isBirdieOrEagle) {
                              spoken = `${holeText}번홀 ${distStr} 미터 지점에서 숏퍼트 미스가 있었습니다. 꼭 넣는 것을 목표로 훈련을 진행해 주세요.`;
                          } else {
                              spoken = `${holeText}번홀 ${distStr} 미터 지점에서 세이브 미스가 있었습니다. 꼭 넣는 것을 목표로 훈련을 진행해 주세요.`;
                          }
                      }
                  }
              } else {
     ...
     if (tType === 'review_hole' || tType === 'review_category') {
         ...
         if (holeCompleted) {
             ...
             if (isAllFinished) {
                     ...
                     try {
                         ...
                         if (chainStr) {
                             ...
                             if (currentIndexInChain !== -1 && currentIndexInChain + 1 < chain.length) {
                                 ...
                                 const nextUrl = `/training/voice-guide?type=review_category&cat=${encodeURIComponent(nextCat.catName)}&holes=${nextCat.encodedHoles}&recordId=${recordId}&isPrep=${isPrep}`;
                                 ...
                                 let completionPhrase = `다음 ${isPrep ? '예습' : '복습'} 훈련을 진행해 주세요.`;
                                 if (currentGroup === "putt") {
                                     completionPhrase = `퍼팅 ${isPrep ? '예습' : '복습'}훈련을 완료하였습니다.`;
                                 } else if (currentGroup === "short") {
                                     completionPhrase = `숏게임 ${isPrep ? '예습' : '복습'} 훈련을 완료하였습니다.`;
                                 } else if (currentGroup === "second") {
                                     completionPhrase = `샷 훈련을 완료하였습니다. 티샷 훈련 이어서 진행하겠습니다.`;
                                 }
                 ...
                 let finalCompletionPhrase = `준비된 모든 ${isPrep ? '예습' : '복습'} 훈련을 완료하였습니다. 수고하셨습니다.`;
                 if (reviewCat) {
                     const cat = reviewCat.toLowerCase();
                     if (cat.includes("티샷") || ["180m이상", "150-179m", "120-149m", "90-119m", "피치샷", "세컨샷", "아이언"].some(g => cat.includes(g))) {
                         finalCompletionPhrase = `티샷 ${isPrep ? '예습' : '복습'} 훈련을 완료하였습니다. 수고하셨습니다.`;
                     }
                 }
    ...
    >
       ...
       <div className="w-full max-w-sm flex flex-col items-center text-center relative">
           ...
           {(() => {
               ...
               if (trainingType === 'lesson_review') return null;
               ...
               return (
                   <div className="w-full flex items-center justify-between mt-2 mb-4 px-4">
                       ...
                       {(trainingType === 'review_hole' || trainingType === 'review_category') && currentSpokenPrompt && (
                (() => {
           ...
           (() => {
               const currentHoleData = trainingType === 'review_category' ? parsedHoles[Math.min(stageIndex, Math.max(0, parsedHoles.length - 1))] : { hole: reviewHole, attempt: reviewAttempt, result: reviewResult, score: reviewScore, note: reviewNote };
               ...
               <div className="w-full bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-700 shadow-sm overflow-hidden mb-10 text-left">
                   ...
                   <div className="p-4 space-y-1">
                       <div className="flex items-center p-3 rounded-xl bg-zinc-50/50 dark:bg-zinc-800/50">
                           <div className="w-1.5 h-6 bg-zinc-300 dark:bg-zinc-600 rounded-full mr-4 shrink-0"></div>
                           <span className="text-lg font-bold text-zinc-400 w-20 shrink-0">시도</span>
                           <span className="flex-1 text-2xl font-semibold text-zinc-700 dark:text-zinc-300 truncate">
                               {currentHoleData.attempt}
                           </span>
                       </div>
                       <div className="flex items-center p-3 rounded-xl bg-rose-50/30 dark:bg-rose-950/10">
                           <div className="w-1.5 h-6 bg-rose-500 rounded-full mr-4 shrink-0"></div>
                           <span className="text-lg font-bold text-zinc-400 w-20 shrink-0">결과</span>
                           <span className="flex-1 text-2xl font-bold text-zinc-900 dark:text-zinc-100 whitespace-nowrap overflow-hidden text-ellipsis">
                       ...
                       {currentHoleData.score && currentHoleData.score !== '-' && (
                           <div className="flex items-center p-3 rounded-xl bg-rose-50/30 dark:bg-rose-950/10">
                               <div className="w-1.5 h-6 bg-zinc-300 dark:bg-zinc-600 rounded-full mr-4 shrink-0"></div>
                               <span className="text-lg font-bold text-zinc-400 w-20 shrink-0">샷별점수</span>
                               <span className={`flex-1 text-2xl font-bold whitespace-nowrap overflow-hidden text-ellipsis ${Number(currentHoleData.score) < 0 ? 'text-red-500' : 'text-blue-500'}`}>
           ...
           <div className="w-full flex justify-between items-end mb-6 px-6 sm:px-8">
               {trainingType !== 'lesson_review' ? (
                   <button
                   ...
                   >
                       <div className="w-[52px] h-[52px] rounded-full bg-white dark:bg-zinc-800 flex items-center justify-center shadow-md text-zinc-700 dark:text-zinc-300 border border-zinc-100 dark:border-zinc-700 group-hover:bg-zinc-50 dark:group-hover:bg-zinc-700">
                           <Lock className="w-5 h-5 text-zinc-500" />
                       </div>
                       <span className="text-[11px] font-bold text-zinc-600 dark:text-zinc-400">화면 잠금</span>
                   </button>
               ) : !isStarted ? (
                   <button
                       ...
                       className="flex flex-col items-center gap-2 active:scale-95 transition-all group"
                   >
                       <div className="w-[52px] h-[52px] rounded-full bg-[#1e293b] flex items-center justify-center shadow-lg group-hover:bg-[#0f172a]">
                           <Play className="w-6 h-6 text-white fill-current ml-1" />
                       </div>
                       <span className="text-[11px] font-bold text-zinc-600 dark:text-zinc-400">훈련 시작</span>
                   </button>
               ) : (
                   <button
                       ...
                       className="flex flex-col items-center gap-2 active:scale-95 transition-all group"
                   >
                       <div className={`w-[52px] h-[52px] rounded-full flex items-center justify-center shadow-md ${isPaused ? 'bg-[#1e293b] text-white' : 'bg-white dark:bg-zinc-800 text-zinc-700 dark:text-zinc-300 border border-zinc-100 dark:border-zinc-700'}`}>
                           {isPaused ? <Play className="w-6 h-6 fill-current ml-1" /> : <Pause className="w-6 h-6 fill-current" />}
                       </div>
                       <span className="text-[11px] font-bold text-zinc-600 dark:text-zinc-400">{isPaused ? '훈련 시작' : '일시 정지'}</span>
                   </button>
               )}
               ...
               <button
                   ...
                   className="flex flex-col items-center gap-2 active:scale-95 transition-all group"
               >
                   <div className="w-[52px] h-[52px] rounded-full bg-white dark:bg-zinc-800 flex items-center justify-center shadow-md text-zinc-700 dark:text-zinc-300 border border-zinc-100 dark:border-zinc-700">
                       <Volume2 className="w-5 h-5" />
                   </div>
                   <span className="text-[11px] font-bold text-zinc-600 dark:text-zinc-400">다시 듣기</span>
               </button>
               ...
               {trainingType !== 'lesson_review' && (
                   <button 
                      ...
                      className="flex flex-col items-center gap-2 active:scale-95 transition-all group"
                   >
                       <div className="w-[52px] h-[52px] rounded-full bg-blue-600 flex items-center justify-center shadow-xl text-white hover:bg-blue-700">
                           <Check className="w-6 h-6" strokeWidth={3} />
                       </div>
                       <span className="text-[11px] font-bold text-blue-600 dark:text-blue-500">완료</span>
                   </button>
               )}
               ...
               <button 
                  ...
                  className="flex flex-col items-center gap-2 active:scale-95 transition-all group"
               >
                   <div className="w-[52px] h-[52px] rounded-full bg-red-100 dark:bg-red-900/30 flex items-center justify-center shadow-md text-red-600 dark:text-red-400 hover:bg-red-200 border border-red-200 dark:border-red-800">
                       <Square className="w-5 h-5 fill-current" />
                   </div>
                   <span className="text-[11px] font-bold text-red-600 dark:text-red-400">훈련 종료</span>
               </button>
           </div>
