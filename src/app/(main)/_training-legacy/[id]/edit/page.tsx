                    ...
                    <section className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 p-6 rounded-2xl shadow-sm space-y-5">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                            ...
                            <div className="space-y-2 md:col-span-2">
                                <label className="block text-sm font-semibold text-zinc-700 dark:text-zinc-300">훈련 유형 <span className="text-brand-red">*</span></label>
                                <div className="flex flex-wrap gap-2 pb-2">
                                    {trainingTypeOptions.map((opt) => (
                                        <button key={opt.key} type="button" onClick={() => setSelectedTrainingType(opt.key)} className={cn("px-4 py-2.5 rounded-xl text-sm font-medium border transition-all", selectedTrainingType === opt.key ? "bg-brand-navy text-white border-brand-navy shadow-sm" : "bg-transparent dark:bg-zinc-800 text-zinc-700 dark:text-zinc-300 border-zinc-200 dark:border-zinc-700 hover:border-brand-navy/30")}>{opt.label}</button>
                                    ))}
                                </div>
                            </div>
                        </div>
