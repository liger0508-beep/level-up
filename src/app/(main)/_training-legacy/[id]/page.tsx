...
import { BookOpen, ChevronLeft, ChevronRight, MoreVertical, Calendar, Download, AlertCircle, MessageSquare, Send, Flag, MapPin, User, CheckCircle2, Check, Edit2, Trash2, Layers, Paperclip, X, Play, ChevronDown, Trophy, AlertTriangle, Target, Video } from "lucide-react";
...
function getTrainingCategories(isPrep: boolean, reviewData: any) {
    ...
    if (isPrep) {
        ...
        const totalScore = reviewData?.recentScore?.score || 72;
        const is9Holes = reviewData?.analysis?.length <= 9 || reviewData?.recentScore?.totalPar === 36;
        let threshold = 0.1;
        
        if (is9Holes) {
            if (totalScore >= 51) threshold = 0.4;
            else if (totalScore >= 46) threshold = 0.3;
            else if (totalScore >= 40) threshold = 0.2;
            else if (totalScore >= 35) threshold = 0.1;
            else threshold = 0.0;
        } else {
            if (totalScore >= 100) threshold = 0.5;
            else if (totalScore >= 90) threshold = 0.4;
            else if (totalScore >= 80) threshold = 0.3;
            else if (totalScore >= 70) threshold = 0.2;
            else threshold = 0.1;
        }

        const hasBadTeeShot = (h: any) => {
            if (h.par < 4) return false;
            const teeShot = (h.shots || []).find((s: any) => s.shotLabel?.split('/')[0].trim().toUpperCase() === "TE");
            if (!teeShot) return false;
            const landing = teeShot.landingLabel || "";
            return !["FW", "GA", "GB", "GR"].includes(landing);
        };
        ...
        const cats = Object.entries(CATEGORY_TO_FIELD).map(([name, field]) => {
            ...
            if (Array.isArray(field)) {
                maxSg = Math.max(...reviewData.analysis.map((h: any) => field.reduce((sum, f) => sum + (h.summary[f] || 0), 0)));
                totalSg = reviewData.analysis.reduce((s: any, h: any) => s + field.reduce((sum, f) => sum + (h.summary[f] || 0), 0), 0);
            } else {
                maxSg = Math.max(...reviewData.analysis.map((h: any) => h.summary[field] || 0));
                totalSg = reviewData.analysis.reduce((s: any, h: any) => s + (h.summary[field] || 0), 0);
            }

            if (name === "티샷") {
                if (reviewData.analysis.some(hasBadTeeShot)) {
                    maxSg = Math.max(maxSg, threshold);
                }
                // Check if any individual tee shot has shotSG >= 0.2
                const hasHighSgTeeShot = reviewData.analysis.some((h: any) => {
                    const teeShots = getCategoryShots("티샷", h, false);
                    return teeShots.some((s: any) => s.shotSG >= 0.2);
                });
                if (hasHighSgTeeShot) {
                    maxSg = Math.max(maxSg, 0.2);
                }
            }
            return {
                name,
        ...
        const positiveCats = cats.filter(c => c.maxSg >= (c.name === "티샷" ? Math.min(0.2, threshold) : threshold)).sort((a: any, b: any) => b.sg - a.sg).sort((a: any, b: any) => {
            const keys = Object.keys(CATEGORY_TO_FIELD);
            return keys.indexOf(a.name) - keys.indexOf(b.name);
        });

        return positiveCats.map(cat => {
            let focusHoles: any[] = [];
            const field = CATEGORY_TO_FIELD[cat.name];
            const currentThreshold = cat.name === "티샷" ? Math.min(0.2, threshold) : threshold;

            if (cat.name === "퍼팅") {
                const fields = field as string[];
                focusHoles = [...reviewData.analysis]
                    .filter(h => {
                        const holeTotalPuttSg = fields.reduce((sum, f) => sum + (h.summary[f] || 0), 0);
                        return holeTotalPuttSg >= currentThreshold;
                    })
                    .flatMap(h => {
                        const relevantShots = getCategoryShots(cat.name, h, false);
                        return relevantShots.map((shot: any) => ({
                            holeNumber: h.holeNumber,
                            hData: h,
                            worstShotInfo: createWorstShotInfo(shot, h),
                            uniqueId: `${cat.name}_${h.holeNumber}_${shot.attemptDistance}`
                        }));
                    });
            } else {
                ...
                focusHoles = [...reviewData.analysis]
                    .filter(h => {
                        if (cat.name === "티샷") {
                            if (hasBadTeeShot(h)) return true;
                            const tShots = getCategoryShots("티샷", h, false);
                            if (tShots.some((s: any) => s.shotSG >= 0.2)) return true;
                        }
                        const holeSg = isArrayField 
                            ? (field as string[]).reduce((sum, f) => sum + (h.summary[f] || 0), 0)
                            : (h.summary as any)[field as string] || 0;
                        return holeSg >= currentThreshold;
                    })
                    .flatMap(h => {
                        const relevantShots = getCategoryShots(cat.name, h, false);
                        if (relevantShots.length === 0) return [];
                        const sortedShots = relevantShots.sort((a: any, b: any) => b.shotSG - a.shotSG);
                        ...
                        return filteredShots.map(shot => ({
                            ...
                            hData: h,
                            worstShotInfo: createWorstShotInfo(shot, h),
                            uniqueId: `${cat.name}_${h.holeNumber}_${shot.attemptDistance}_${shot.shotNumber || 0}`
                        }));
                    });
            }
            ...
            } else if (cat.name === "아이언&피치샷") {
                focusHoles.sort((a: any, b: any) => a.worstShotInfo.rawAttemptDistance - b.worstShotInfo.rawAttemptDistance);
            }

            return { name: cat.name, holes: focusHoles };
        }).filter(cat => cat.holes.length > 0);
    }
}
        ...
        <div className="min-h-screen bg-zinc-50 dark:bg-zinc-950 pb-24">
                                ...
                                <div className="space-y-3">
                                    ...
                                    return (
                                        <div key={catName} className={cn("border rounded-2xl overflow-hidden shadow-sm transition-colors", isAllCompleted ? "border-indigo-200 dark:border-indigo-800/50" : "border-zinc-200 dark:border-zinc-800")}>
                                            ...
                                            {isExpanded && (
                                                <div className="p-4 bg-zinc-50 dark:bg-zinc-800/30 border-t border-zinc-200 dark:border-zinc-800 relative group">
                                                    <div className="mb-3">
                                                        <div className="text-xs font-bold text-zinc-500 dark:text-zinc-400 flex items-center gap-1.5 mb-3.5">
                                                            <Flag size={14} className={isAllCompleted ? "text-indigo-400" : "text-brand-navy/60 dark:text-brand-navy-light/60"} />
                                                        ...
                                                        {(() => {
                                                            ...
                                                            const colCountMobile = totalHoles >= 10 ? Math.ceil(totalHoles / 2) : Math.max(1, totalHoles);
                                                            
                                                            const renderHole = (h: any) => {
                                                                const hKey = h.uniqueId || `${catName}_${h.holeNumber}`;
                                                                const hCompleted = reviewData.completedHoles.includes(hKey as any);
                                                                return (
                                                                    <div key={hKey} 
                                                                        onClick={() => {
                                                                            setSelectedFocusHole(hKey);
                                                                            const container = document.getElementById(`scroll-container-${idx}`);
                                                                            const card = document.getElementById(`card-${idx}-${h.holeNumber}`);
                                                                            if (container && card) {
                                                                                container.scrollTo({ left: card.offsetLeft - container.offsetLeft, behavior: 'smooth' });
                                                                            }
                                                                        }}
                                                                        className={cn(
                                                                        "relative w-7 h-7 sm:w-8 sm:h-8 rounded-full flex shrink-0 items-center justify-center text-[11px] sm:text-[12px] font-bold border transition-all cursor-pointer hover:scale-110 active:scale-95",
                                                                        hCompleted 
                                                                            ? "bg-zinc-200 text-zinc-400 border-zinc-200 dark:bg-zinc-800 dark:text-zinc-600 dark:border-zinc-800 opacity-70" 
                                                                            : "bg-white text-zinc-900 border-zinc-300 shadow-sm dark:bg-zinc-900 dark:text-zinc-100 dark:border-zinc-700"
                                                                    )}>
                                                                        {h.holeNumber}
                                                                        {hCompleted && (
                                                            ...
                                                            return (
                                                                <>
                                                                    <div 
                                                                        className="grid sm:hidden gap-1.5 w-fit"
                                                                        style={{ gridTemplateColumns: `repeat(${colCountMobile}, minmax(0, 1fr))` }}
                                                                    >
                                                                        {sortedHoles.map(renderHole)}
                                                                    </div>
                                                    ...
                                                    <button onClick={(e) => { e.preventDefault(); e.stopPropagation(); document.getElementById(`scroll-container-${idx}`)?.scrollBy({ left: -300, behavior: 'smooth' }) }} className="hidden md:flex absolute left-0 top-1/2 -translate-y-1/2 z-20 w-8 h-8 items-center justify-center bg-white/80 dark:bg-black/80 rounded-full shadow-md text-zinc-600 dark:text-zinc-300 opacity-0 group-hover:opacity-100 transition-opacity hover:bg-white dark:hover:bg-black hover:scale-110 -ml-2">
                                                        <ChevronLeft size={20} />
                                                    </button>
                                                    <button onClick={(e) => { e.preventDefault(); e.stopPropagation(); document.getElementById(`scroll-container-${idx}`)?.scrollBy({ left: 300, behavior: 'smooth' }) }} className="hidden md:flex absolute right-0 top-1/2 -translate-y-1/2 z-20 w-8 h-8 items-center justify-center bg-white/80 dark:bg-black/80 rounded-full shadow-md text-zinc-600 dark:text-zinc-300 opacity-0 group-hover:opacity-100 transition-opacity hover:bg-white dark:hover:bg-black hover:scale-110 -mr-2">
                                                        <ChevronLeft size={20} className="rotate-180" />
                                                    </button>
                                                    <div id={`scroll-container-${idx}`} className="flex overflow-x-auto gap-3 pb-2 snap-x snap-mandatory scrollbar-hide scroll-smooth">
                                                        {focusHolesForCat.filter((h: any, index: number, self: any[]) => index === self.findIndex((t) => t.holeNumber === h.holeNumber)).sort((a: any, b: any) => {
                                                            const aKey = a.uniqueId || `${catName}_${a.holeNumber}`;
                                                            const bKey = b.uniqueId || `${catName}_${b.holeNumber}`;
                                                            const aCompleted = reviewData.completedHoles.includes(aKey as any);
                                                            const bCompleted = reviewData.completedHoles.includes(bKey as any);
                                                            ...
                                                            return (
                                                                <div 
                                                                    ...
                                                                    onClick={() => {
                                                                        ...
                                                                        setConfirmDialog({
                                                                            isOpen: true,
                                                                            message: isCompleted ? '재훈련하시겠습니까?' : '훈련을 시작하시겠습니까?',
                                                                            onConfirm: () => {
                                                                                setConfirmDialog(null);
                                                                                executeTraining(isCompleted);
                                                                            }
                                                                        });
                                                                    }}
                                                                    className={`snap-start shrink-0 w-[220px] sm:w-[240px] rounded-2xl border border-zinc-200 dark:border-zinc-700 shadow-sm p-4 flex flex-col relative overflow-hidden cursor-pointer hover:border-brand-navy dark:hover:border-brand-navy-light hover:shadow-md transition-all active:scale-[0.98] text-left group/card ${isCompleted ? 'bg-zinc-100 dark:bg-zinc-800/50 opacity-80' : 'bg-white dark:bg-zinc-900'}`}
                                                                >
                                                                    {isCompleted && (
                                                                        <div className="absolute top-0 right-0 border-b border-l border-zinc-200 dark:border-zinc-700 bg-zinc-200 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-400 px-3 py-1 rounded-bl-xl text-[10px] font-bold flex items-center gap-1 z-10">
                                                                            <CheckCircle2 size={12} /> 완료
                                                                        </div>
                                                                    )}
                                                                    <div className="mb-3">
                                                                        <span className="text-sm font-black text-zinc-900 dark:text-white group-hover/card:text-brand-navy transition-colors truncate block">
                                                                            {displayHole}번 Hole - Par{h.hData ? h.hData.par : (h.par || 4)}
                                                                        </span>
                                                                    </div>
                                                                    
                                                                    <div className="flex justify-between items-end mt-auto pt-2">
                                                                        <div className="space-y-1.5 flex-1 pr-2 overflow-hidden">
                                                                        ...
                                                                        <div className={cn("w-7 h-7 rounded-full bg-[#4a5e7b] dark:bg-brand-navy flex items-center justify-center text-white shrink-0 shadow-sm transition-all duration-200", selectedFocusHole === uniqueKey ? "scale-110 opacity-100" : "opacity-0 group-hover/card:opacity-100 group-hover/card:scale-110")}>
                                                                            <ChevronLeft size={14} className="rotate-180 ml-0.5" />
                                                                        </div>
                                                                    </div>
                                                                </div>
                                                            );
            ...
            {confirmDialog && confirmDialog.isOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
                    <div className="bg-white dark:bg-zinc-900 rounded-2xl shadow-xl w-full max-w-[320px] overflow-hidden animate-in zoom-in-95 duration-200">
                        <div className="p-6 text-center">
                            <h3 className="text-lg font-bold text-zinc-900 dark:text-white mb-2 whitespace-pre-line leading-relaxed">
