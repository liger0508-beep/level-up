...
import { CourseInfo, CourseInfoType, getCourseInfos, COURSE_INFO_TYPE_LABELS, getPlainText, COURSE_INFO_TYPE_COLORS } from "@/lib/course-info-sync";
import { CourseInfoTable } from "@/components/courseInfo/CourseInfoTable";
...
export default function CommunityPage() {
    ...
    const [activeType, setActiveType] = useState<CourseInfoType>("course_info");
    ...
    useEffect(() => {
        setHasMounted(true);
        getCourseInfos().then(setCourseInfos);
    }, []);
    ...
    // Filtered courseInfos for the main list
    const filteredCourseInfos = useMemo(() => {
        return courseInfos.filter((n) => {
            const typeMatch = n.type === activeType || n.type === "course_info";
            const titleMatch = !searchQuery || n.title.toLowerCase().includes(searchQuery.toLowerCase()) || n.content.toLowerCase().includes(searchQuery.toLowerCase());
    ...
    }, [courseInfos, activeType, searchQuery, startDate, endDate]);

    // Recent 3 courseInfos for the top carousel
    const recentCourseInfos = useMemo(() => {
        return [...courseInfos]
            .sort((a, b) => b.date.localeCompare(a.date))
            .slice(0, 3);
    }, [courseInfos]);

    const displayedCourseInfos = useMemo(() => {
        return filteredCourseInfos.slice(0, displayLimit);
    }, [filteredCourseInfos, displayLimit]);
    ...
    return (
        <div className="p-4 sm:p-8 max-w-4xl mx-auto">
            ...
            {/* ── Recent CourseInfos (Top Carousel) ── */}
            <div className="mb-4">
                ...
                <div className="relative group/scroll">
                    ...
                    >
                        {recentCourseInfos.map((courseInfo) => (
                            <Link
                                key={courseInfo.id}
                                href={`/course-info/${courseInfo.id}`}
                                className={cn(
                            ...
                            >
                                <div className="flex items-center justify-between mb-2">
                                    <span className={cn(
                                        ...
                                        COURSE_INFO_TYPE_COLORS[courseInfo.type].bg,
                                        COURSE_INFO_TYPE_COLORS[courseInfo.type].text
                                    )}>
                                        {COURSE_INFO_TYPE_LABELS[courseInfo.type]}
                                    </span>
                                    {courseInfo.isImportant && (
                                ...
                                <div className="text-[11px] text-zinc-400 font-medium flex items-center justify-end gap-2 mt-auto pt-2 border-t border-zinc-50 dark:border-zinc-800/50">
                                    <span>{courseInfo.author || "알 수 없음"}</span>
                                    <span className="opacity-30">|</span>
                                    <span>{courseInfo.date.replace(/-/g, ".")}</span>
                                </div>
                            </Link>
                        ))}
                    </div>
            ...
            <section>
                ...
                <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-5 shadow-sm">
                    {displayedCourseInfos.length > 0 ? (
                        <>
                            <CourseInfoTable courseInfos={displayedCourseInfos} />
                            {filteredCourseInfos.length > displayLimit && (
                                <div className="mt-8 flex justify-center">
                                    ...
                                    >
                                        더 보기 ({filteredCourseInfos.length - displayLimit}건 남음)
                                    </button>
                                </div>
                            )}
                        </>
